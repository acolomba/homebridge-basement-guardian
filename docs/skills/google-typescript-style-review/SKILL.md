...
This skill is the review-time counterpart of the `google-typescript-style-rule.md` Claude Code rule; the two state the same rules and must not disagree. Points the template's configuration decides are marked *(template)*. When revising, produce the rule file's **Good** form.
