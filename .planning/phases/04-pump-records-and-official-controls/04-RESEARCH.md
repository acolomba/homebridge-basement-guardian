...
1. **Does `update/rejected` belong in the fake broker at all?**
   ...
   - Recommendation: **do not add the subscription, and do not add the leaf.** A fake that publishes a message the real system would never send to a client that would never be listening is the definition of `04-CONTEXT.md` D-15's own warning — "a fake we author answers our own design". Drive all five CTRL-05 outcomes from the REST layer, where the plugin actually observes them, and where the wire shapes are measured. **Raise this with the user before planning**, since D-15 is locked and this reading narrows it to: extend the broker to let the fake pump *react* to an accepted command by reporting `test_running: true` on `update/accepted`, and drop the `update/rejected` leaf entirely.
