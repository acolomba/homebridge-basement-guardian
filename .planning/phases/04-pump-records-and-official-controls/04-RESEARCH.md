...
| Rejecting a command in Cucumber | A shadow `update/rejected` publish | `fakeRestApi.failNextWith(status)` or a `{ success: false }` body | Both are what the plugin actually observes; the shadow rejection leaf is not subscribed (see Open Question 1). |
...
1. **Does `update/rejected` belong in the fake broker at all?**
   ...
   - What's unclear: a publish on a topic the plugin does not subscribe to is unobservable, so a scenario built on it would assert nothing. Adding the subscription would contradict SYNC-02.
   - Recommendation: **do not add the subscription.** Add the leaf to the fake only if the phase writes a *negative* scenario — "a shadow update rejection reaches the broker and the plugin's canonical state is unchanged" — which is a real property worth pinning and needs no plugin change. Drive the five CTRL-05 outcomes entirely from the REST layer, where the plugin actually observes them. **Raise this with the user before planning**, since D-15 is a locked decision and this reading narrows it.
