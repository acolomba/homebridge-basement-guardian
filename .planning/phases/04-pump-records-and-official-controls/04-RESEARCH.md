...
- [ ] `features/support/fakeShadowBroker.ts` extension — the fake pump's *reaction*: an accepted command flips `test_running` and the broker reports it on `update/accepted`. **No `update/rejected` leaf** *if* Open Question 1 is ratified; `D-15` as written still binds until then.
