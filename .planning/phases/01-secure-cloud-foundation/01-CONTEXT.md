...
- **D-09:** Unit tests compile then run: `tsc` emits, `node --test` runs the emitted JavaScript with source maps for stack traces. This preserves the full TypeScript language. Native strip-only mode was rejected on verified evidence — Node rejects `TypeScript parameter property is not supported in strip-only mode`, which would force constructor DI away from parameter properties. **Correction (pattern mapping, 2026-08-28):** the original rationale also cited `const enum`, but `.claude/rules/typescript-style-guide.md:193` bans `const enum` outright, so that half of the argument is void — the decision stands on parameter properties alone. `const enum` is not returning: when Phase 3 needs stable service subtypes, use an `as const` object plus a derived union type. Node also rejects enums in strip-only mode, so both rules point the same way. A second tsconfig emits `test/` alongside `src/`. — **Reversibility:** costly — switching later means touching every test file's build path plus CI.
