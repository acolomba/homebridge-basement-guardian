...
- [x] **RES-01**: Communication or field-validation failure preserves the last valid value and marks only the narrowest affected service stale, inactive, or faulty; omitted partial fields and invalid updates never clear active safety conditions. The device heartbeat is approximately 898 seconds, and approximately 15 minutes of shadow silence is normal. Shadow silence is a secondary staleness signal only after two missed heartbeats, and one missed heartbeat is never evidence that the device is offline.
  ...
  - *Narrowed 2026-09-04 by phase 05.1. The row stays `Complete`.* Two things this clause promises were true of the account and not of the service. **Narrowest scope:** shadow silence withdrew trust from every system on the account, so one quiet pump stopped its neighbours vouching; plan 05.1-02 made the answer per device end to end, and plan 05.1-03 made the recovery latch and the `LIVE_REPORTING` failure log per device with it. The mutation that fails it is restoring the flatten — `shadowSilent: silentDevices().length > 0` — which reds `features/degradedOperation.feature:265`. **Preserved rather than refreshed:** the silence window was measured on the wall clock, so a backwards correction handed a dead pump a fresh certificate of health; plan 05.1-04 measures it as the larger of a forward-only elapsed time and a wall elapsed time, and plan 05.1-05 persists the wall anchor so a restart cannot re-vouch for a pump that was already quiet. The mutations are inverting the clamp to `Math.min` and letting admission overwrite a restored anchor. **The limit, stated rather than softened:** the vendor account holds exactly one Gemini, so every per-pump claim rests on the Cucumber harness and on those mutations, not on hardware.
- [x] **RES-02**: `serial_communications === false` immediately activates `Pump Controller Link Lost`, faults controller-derived services, preserves their values, and exposes when trustworthy controller data was last received.
  - *Completed 2026-09-04 by phase 05.1, plan 05.1-06. The row stays `Complete`.* The clause's "faults controller-derived services" was satisfied in behaviour and not in what the plugin told an owner: the warning named five scope words while the marking pass withdrew fifteen published services. The line is now derived from the descriptors the accessory just published, joined to their catalogue rows on kind and HAP service type identifier, and filtered by the non-connectivity scopes minus the one row that tolerates this cause. It cannot drift from the marking pass: the mutation that fails it is changing the catalogue without touching the message, which reds three pinned renderings. **Corrected here:** the figure "marks 16 and tells the operator about 11", carried by `STATE.md` and the Phase 5 handoff, is wrong in both halves and 11 is not derivable from any source. Measured against the shipped catalogue: 17 rows, 16 non-connectivity, 15 that stop vouching, across 7 scopes.
...
- [x] **REL-02**: Deterministic `node:test` unit tests and Cucumber fake-pump tests cover reducers, decoders, family adapters, events, health, reconciliation, commands, accessories, privacy, and runtime lifecycles. They use sanitized fixtures without live credentials, hardware, public network access, or stored secrets.
- [x] **REL-03**: Runtime traffic is limited to required vendor services; the plugin collects no analytics, crash reports, installation/usage data, tracking identifiers, or automatic diagnostics, and direct dependencies pass a telemetry review.
- [x] **REL-04**: Public, planning, fixture, log, and packed-package checks exclude secrets and account/local-network identifiers and use stable placeholders, while the bundled data file alone may contain required public vendor constants.
- [x] **REL-05**: The packed package consistently preserves MIT licensing for original standalone work, Apache 2.0 licensing and notices for template-derived material, required file headers, full license texts, and aligned `SEE LICENSE IN LICENSE` metadata while both remain.
...
- [x] **REL-09**: An opt-in Cucumber suite observes a real pump's discovery, initial state, heartbeats, natural updates, restart, and shutdown. The suite blocks all commands in code, uses local credentials, and never requires a natural status change.
